...
describe('GET /api/risk', () => {
  it('returns a portfolio risk assessment scoped to the client', async () => {
    ...
    expect(json.risk.totalSuppliers).toBe(1)
    expect(json.risk.totalPlaces).toBe(1)
    expect(json.risk.distribution.high).toBe(1)
    expect(json.risk.conclusion).toBe('action_required')
    expect(json.risk.mitigationNeeded).toBe(1)
